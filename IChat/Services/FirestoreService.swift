...
//  IChat
//
//  Created by Михаил Красильник on 21.04.2021.
//
...
class FirestoreService {
        ...
        docRef.getDocument { (document, error) in
            if let document = document, document.exists {
                guard let mUser = MUser(document: document) else {
                    completion(.failure(UserError.canNotUnwrapeToMUser))
                    return
                }
    ...
    func createWaitingChat(message: String, receiver: MUser, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        reference.document(currentUser.id).setData(chat.representation) { (error) in
            ...
            messageRef.addDocument(data: message.representaztion) { (error) in
                if let error = error {
                    completion(.failure(error))
                    return
                }
    ...
    func deleteaitingChat(chat: MChat, completion: @escaping (Result<Void, Error>) -> Void) {
    ...
    func changeToActive(chat: MChat, completion: @escaping (Result<Void, Error>) -> Void) {
        getWaitingChatsMessages(chat: chat) { result in
            ...
            case .success(let messages):
                self.deleteaitingChat(chat: chat) { result in
                    switch result {
                    case .success():
    ...
    func sendMessage(chat: MChat, message: MMessage,  completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        let userMessagesRef = usersRef.document(currentUser.id).collection("activeChats").document(chat.friendId).collection("messages")
        ...
        friendRef.setData(chatForFriend.representation) { error in
            ...
            friendMessagesRef.addDocument(data: message.representaztion) { error in
                if let error = error {
                    completion(.failure(error))
                    return
                }
                userMessagesRef.addDocument(data: message.representaztion) { error in
                    if let error = error {
                        completion(.failure(error))
                        return
                    }
